...
from api.status_client import StatusClient
...
class DocumentProcessor:
    ...
    4. Update job status via Go backend API
    """
    ...
    async def _process_internal(self, job: IndexingJobMessage) -> ProcessingResult:
        ...
        try:
            ...
            # 4. Update request completed jobs counter (jobs are not stored in DB individually)
            await self.status_client.increment_completed_jobs(job.request_id)
            ...
            # Note: We don't update job status on failure since jobs aren't stored in DB
            # The request will show incomplete when total_jobs != completed_jobs
